...
class PublicController extends AdminBaseController
{
    ...
    public function choosePortal(){
        $id = input('id', 0);

        $portal = model('index/main/portal')->where('id', $id)->field('name')->find();

        session('uniacid', $id);
        $this->result($portal, 1);
    }
    ...
    public function login()
    {
        ...
        /*if (empty($loginAllowed)) {
            ...
            return redirect(cmf_get_root() . "/admin/public/login");
        }*/
    ...
    public function doLogin()
    {
        ...
        if (!empty($result) && $result['user_type'] == 1) {
            if (cmf_compare_password($pass, $result['user_pass'])) {
                ...
                $portal = PortalModel::getDefault($result);
                if(empty($portal) && $result["id"] != 1){
                    session('ADMIN_ID', null);
                    $this->error('您没有门户管理权限，请联系管理员');
                }
                
                //登入成功页面跳转
                session('uniacid', $portal['id']);
